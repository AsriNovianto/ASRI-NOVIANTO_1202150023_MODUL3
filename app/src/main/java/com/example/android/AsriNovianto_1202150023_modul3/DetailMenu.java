...
public class DetailMenu extends AppCompatActivity {
    ...
    //Pada langka ini kita akan memberikan notifikasi bila air sudah penuh (6) maka akan muncul notifikasi full,bila air kosong(0) maka akan muncul notifikasi empty

    public void tambahIsi(){
        status();
        if(level==6){Toast.makeText(this,"Air yang ada digalon sudah penuh",Toast.LENGTH_SHORT).show();return;}
        progress.setImageLevel(++level);
    }

    public void kurangIsi(){
        status();
        if(level==0){Toast.makeText(this,"Air yang ada digalon sudah kosong",Toast.LENGTH_SHORT).show();return;}
        progress.setImageLevel(--level);
    }
